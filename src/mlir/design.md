...
We lower all static and dynamic calls as they are, and declare those functions that weren't defined yet.
At the end, all user and compiler-generated functions are defined, except for the arithmetic ones.
The generator has to find those that weren't declared and replace with operators if the types are compatible (int, float, etc).
It's an error to have any undefined functions after that.
...
class U32 {}
class Foo {
  static +(a: U32, b: U32) : U32 { ... }
}
main() : U32 {
  let a : U32 = 21 + 21; // This is simple arithmetic
  let b : U32 = Foo::+(21, 21); // This calls the method
}
```
