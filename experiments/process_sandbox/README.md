...
-----------
 ...
 - [child_malloc.h](child_malloc.h) contains the interfaces for the parts that specialise snmalloc for use in the child.
 - [cxxapi](cxxapi) contains the C++ API, which is primarily used for testing.
 - [filetree.h](filetree.h) describes the interface for exporting a virtual file tree to the child.
   This is accessed from the `SandboxedLibrary` class.
 - [helpers.h](helpers.h) provides some helpers for managing C memory, extracting argument types from functions, and so on.
 - [host_service_calls.h](host_service_calls.h) describes the IPC mechanism used for the snmalloc to request memory and update the sandbox.
 - [library_runner.cc](library_runner.cc) is the program that runs inside the sandbox that loads the library and manages communication with the parent.
 - [libsandbox.cc](libsandbox.cc) is the library that manages sandbox lifecycle.
 - [platform/](platform) contains the platform abstraction code.
 - [privilege_elevation_upcalls.h](privilege_elevation_upcalls.h) describes the callback mechanism.
 - [sandbox_fd_numbers.h](sandbox_fd_numbers.h) contains the file descriptors that are set on child-process creation.
 - [sandbox.hh](sandbox.hh) contains the definition of the sandbox library interface.
