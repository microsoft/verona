...
int main(void)
{
  ...
  SANDBOX_INVARIANT(sp.first.nonblocking_send(i), "Send failed");
  i = 12;
  // Check receiving succeeds and gives us the value we wanted
  SANDBOX_INVARIANT(sp.second.nonblocking_receive(i), "Receive failed");
  ...
  Handle h;
  SANDBOX_INVARIANT(sp.second.nonblocking_receive(i, h), "Receive failed");
  ...
  SANDBOX_INVARIANT(sp.first.nonblocking_send(i, h), "Sending a socket failed");
  SANDBOX_INVARIANT(
    sp.second.nonblocking_receive(i, h), "Receiving a socket failed");
  ...
  SANDBOX_INVARIANT(
    sp2.first.nonblocking_send(i), "Sending over received socket failed");
  i = 12;
  SocketPair::Socket newsock;
  newsock.reset(h.take());
  SANDBOX_INVARIANT(
    newsock.nonblocking_receive(i), "Receiving over received socket failed");
